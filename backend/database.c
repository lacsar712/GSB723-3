...
static void rule_dispute(Dispute *d, int upheld, const char *result_text) {
...
void save_data() {
  ...
  log_message(LOG_INFO, "Data saved (orders=%d users=%d ratings=%d disputes=%d)",
              order_count, user_count, rating_count, dispute_count);
}
